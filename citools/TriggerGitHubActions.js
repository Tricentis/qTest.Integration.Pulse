/**
 * Dispatches a GitHub Actions workflow and optionally emits ChatOpsEvent.
 *
 * Event: { correlationId?: string, ref?: string, inputs?: object }
 * Required constants: GitHubOwner, GitHubRepository, GitHubWorkflow,
 * GitHubToken, and GitHubRef unless event.ref is supplied.
 * Optional constant: CI_REQUEST_TIMEOUT_MS (default 15000, maximum 45000)
 *
 * The workflow must declare the `workflow_dispatch` event.
 */

const RULE_NAME = "TriggerGitHubActions";
...
exports.handler = async function ({ event: body, constants, triggers }, context, callback) {
    ...
    try {
        ...
        workflow = `${config.owner}/${config.repository}:${config.workflow}`;
        const requestUrl = `https://api.github.com/repos/${encodeURIComponent(config.owner)}/${encodeURIComponent(config.repository)}` +
            `/actions/workflows/${encodeURIComponent(config.workflow)}/dispatches`;
        const requestBody = { ref: config.ref };
        if (config.inputs) requestBody.inputs = config.inputs;

        writeLog("INFO", "Dispatching GitHub Actions workflow.", {
            correlationId: correlationId, stage: "dispatch", provider: PROVIDER,
            pipeline: workflow, ref: config.ref, inputCount: config.inputs ? Object.keys(config.inputs).length : 0,
        });
        ...
        ensureSuccessfulResponse(response, "GITHUB_ACTIONS_HTTP_ERROR", "GitHub Actions");
        ...
        const result = {
            ...
            runId: response.data && (response.data.workflow_run_id || response.data.id),
            runUrl: response.data && (response.data.html_url || response.data.run_url),
        };
        writeLog("INFO", "GitHub Actions accepted the workflow dispatch.", {
            correlationId: correlationId, stage: "dispatch", provider: PROVIDER, pipeline: workflow,
        ...
        const wrapped = normalizeRequestError(error, "GITHUB_ACTIONS_REQUEST_FAILED", "GITHUB_ACTIONS_TIMEOUT", constants);
        writeLog("ERROR", "Unable to dispatch the GitHub Actions workflow.", Object.assign({
            correlationId: correlationId, stage: "dispatch", provider: PROVIDER, pipeline: workflow,
            durationMs: Date.now() - startedAt,
        }, getSafeErrorFields(wrapped)));
        await emitOptionalChatOps(triggers, createChatOpsPayload({
            correlationId: correlationId, provider: PROVIDER, pipeline: workflow,
...
function getConfig(body, constants) {
    ...
    if (inputs !== undefined && (!inputs || typeof inputs !== "object" || Array.isArray(inputs))) {
        throw createRuleError("CONTRACT_INVALID", "GitHub Actions event 'inputs' must be an object.");
    }
    if (inputs && Object.keys(inputs).length > 25) {
        throw createRuleError("CONTRACT_INVALID", "GitHub Actions supports at most 25 workflow inputs.");
    }
    return {
        owner: requireConstant(constants, "GitHubOwner"),
        repository: requireConstant(constants, "GitHubRepository"),
        workflow: requireConstant(constants, "GitHubWorkflow"),
        token: requireConstant(constants, "GitHubToken"),
        ref: requireEventOrConstant(body, "ref", constants, "GitHubRef"),
        inputs: inputs,
...
function createChatOpsPayload(result, outcome, error) {
    ...
    return {
        ...
        message: outcome === "accepted"
            ? `[INFO] correlationId=${result.correlationId} GitHub Actions accepted workflow '${pipeline}'${formatRunSuffix(result.runId)}.`
            : `[ERROR] correlationId=${result.correlationId} GitHub Actions failed to dispatch workflow '${pipeline}' (${error.code || "UNKNOWN"}).`,
...
function requireEventOrConstant(body, eventName, constants, constantName) {
    ...
    if (value === undefined || value === null || String(value).trim() === "") {
        throw createRuleError("CONFIG_INVALID", `Provide event.${eventName} or Pulse constant '${constantName}'.`);
    }
    return String(value).trim();
}
