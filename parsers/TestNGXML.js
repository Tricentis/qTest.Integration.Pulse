    ...
    xml2js.parseString(testResults, {
        ...
        for (const ts of testSuites) {
            ...
            for (const tc of testClasses) {
                ...
                const invalidMethods = ['endtest', 'setup', 'suitesetup', 'beforesuite', 'aftermethod', 'beforemethod', 'beforetest', 'beforeclass', 'afterclass', 'aftersuite', 'aftertest', 'testbreakdown', 'reset', 'teardown'];
                    ...
                    if (!invalidMethods.includes(methodName.toLowerCase())) {
                        ...
                        if (methodStatus == 'FAIL') {
                            ...
                            testLog.attachments.push({
                                'name': `stack-trace-${methodName}-${exe_start_date.toString().replace(/:/g,'-')}.txt`,
                ...
                if (testLogs.length > 0) {
                    ...
                    console.log('[INFO] Class had 0 valid test methods (likely all housekeeping tasks), skipping...');
                }
            }
        }
    });
