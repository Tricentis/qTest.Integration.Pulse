/**
 * Pulse usage: Parses consolidated Cypress Mochawesome JSON delivered by
 * delivery.js.
...
const RULE_NAME = "CypressMochawesomeJSON";
...
exports.handler = async function ({ event: body, constants, triggers }, context, callback) {
    ...
    try {
        ...
        const destination = getSubmissionDestination(payload);
        ...
        writeLog("INFO", "Cypress results parsed successfully.", {
            correlationId: correlationId,
            stage: "parse",
            targetType: destination.targetType,
            targetId: destination.targetId,
        ...
        writeLog("ERROR", "Unable to process Cypress results.", {
            correlationId: correlationId,
            stage: "failed",
        ...
        await emitEvent("ChatOpsEvent", {
            correlationId: correlationId,
            errorCode: error && error.code ? error.code : "PARSE_FAILED",
            message: `[ERROR] correlationId=${correlationId} Unable to process Cypress results: ${error.message}`,
        });
        throw error;
    }
};
...
function validatePayload(payload) {
    if (!payload || typeof payload !== "object") {
        throw createParserError("CONTRACT_INVALID", "The Pulse event payload is missing.");
    }
    if (payload.projectId === undefined || payload.projectId === null || payload.projectId === "") {
        throw createParserError("CONTRACT_INVALID", "The Pulse event payload is missing 'projectId'.");
    }
...
function getSubmissionDestination(payload) {
    ...
    return {
        targetType: hasTestSuite ? "test-suite" : "test-cycle",
